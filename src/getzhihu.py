...
question_data = re.findall('<div class="HotItem-content">.*?title="(.*?)" target', page_data, re.S)
lianjie=re.findall('<div class="HotItem-content">.*?href="(.*?)" title',page_data,re.S)
print(lianjie)
...
dest_filename='知乎热榜'+time_hot+'.xlsx'
...
for i in range(len(question_data)):
    ...
    ws.cell(row=i+2, column=2, value=question_data[i])
    ws.cell(row=i+2,column=3,value=lianjie[i])
